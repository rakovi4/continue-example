...
|---|---|---|---|
| T1 Instrumentation + ceiling benchmark | Timestamp every agent dispatch/return; per-step time-budget table; ceiling runs (D4) on both fixtures, old vs new | — | Unblocks all estimates |
...
| T5 Coverage-red fan-out + cross-unit overlap (1.6) | N gaps → N parallel red agents, 1 green after; red agents start while original green's refactor+review batch still runs | T2 | Medium (~5–10%) |
